...
int key_pin = 7;    //配网按键 S3 KEY_SW2 与Arduino相连
...
unsigned char pid[] = {"6t5xhq50s3vfihcd"};//*********处替换成涂鸦IoT平台自己创建的产品的PID 小哈哥幻彩灯带-15
...
void visualize_music() {
  ...
  //light_led_count = sensor_value%PIXEL_COUNT;
  //Serial.println(light_led_count);

  if(light_led_count>PIXEL_COUNT)
    light_led_count=PIXEL_COUNT;
  ...
  switch(dpid) {
    ...
    break;
     ...
     switch(scene_mode){
      ...
      break;
      case DPID_DREAMLIGHTMIC_MUSIC_DATA: //幻彩灯带外置麦克风音乐律动
        my_device.mcu_dp_update(dpid, value, length);
        break;
